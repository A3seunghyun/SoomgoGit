...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
	// 	String servletType = request.getAttribute("servletType").toString();
...
// 		// 내 idx(로그인으로 파악)
// 		int my_idx = Integer.parseInt(request.getAttribute("myIdx").toString());
// 		// 채팅방 idx 
// 		int chatroom_idx = Integer.parseInt(request.getAttribute("chatroomIdx").toString());
...
// 	// 채팅방 idx 해당하는 채팅 내역 리스트
// 	UsersDAO uDAO = new UsersDAO();
	//ArrayList<ChatContentsDTO> listChatContents = (ArrayList<ChatContentsDTO>) request.getAttribute("myList");
%>
<!DOCTYPE html>
<html>
<head>
...
<title>숨고 채팅방 -승현</title>
	...
	<link rel="stylesheet" href="<%=request.getContextPath()%>/soomgo_chat_room.css">
...
// 		상단 검색바 여기까지-----------------------------------------------------------------------------------------------------------------------------------------
		...
		let submitFunction = function() {
			// servlet으로 이미지 또는 파일 전송
		}
		$(".user-chat-form").on({click: function(){ submitFunction(); }}, {keydown})
	...
	<%
		...
		// 채팅방 idx 해당하는 채팅 내역 리스트
		UsersDAO uDAO = new UsersDAO(); 
		GosuinforDTO gDTO = uDAO.getGosuInfoByUsersIdx(users_idx);
		GosuServiceTownDTO gsDTO = uDAO.getGosuServiceTown(users_idx);
	%>
</head>
<body>
								...
								<div style="max-height: 731px">
									<div class="chat-contents-container">
										<ul>
												...
												<div class="message-box">
													<div class="user-profile-img">
														<div class="chat-user-profile-picture">
															<div class="image">
																<img src="<%= uDAO.getGosuInfoByUsersIdx(users_idx).getF_img() %>">
															</div>
														</div>
													</div>
											...
											<%	} else { // 내 채팅 %>	
											<li class="is-quote-msg my_chat_li">
												<div class="message-box">
													<div class="message-body">
													...
													<div class="user-profile-img right_position">
														<div class="chat-user-profile-picture">
															<div class="image">
																<img src="<%= uDAO.getGosuInfoByUsersIdx(users_idx).getF_img() %>">
															</div>
									...
									<div class="col-auto1">
										<div class="file-attach-btn-container">
											<ul class="chat-function-buttons">
												<li class="add-file">
													<input type="file" name="my-file" style="display:none"/>
