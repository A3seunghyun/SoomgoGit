...
<%@page import="dto.ServiceList"%>
<%@page import="dao.ServiceDao"%>
<%@page import="dto.NoticeDto"%>
<%@page import="dto.MainRandomGosuDto"%>
<%@page import="dao.MainDao"%>
<%@page import="dto.CategoryDto"%>
<%@page import="dao.CategoryDao"%>
<%@page import="dto.MainPostActionDto"%>
<%@page import="dto.MainpostListDto"%>
<%@page import="dao.PostListDao"%>
<%@page import="dto.GosuKnowhowPostListDto"%>
<%@page import="java.util.ArrayList"%>
<%@page import="dao.GosuKnowhowPostDao"%>
...
<%
	...
	GosuKnowhowPostDao gDao = new GosuKnowhowPostDao();

	//메인페이지 고수노하우 최근작성 순 상위 2개 포스트 뿌리기
	ArrayList<GosuKnowhowPostListDto> mainList = gDao.getGkhPostList();
	
	// 메인페이지 게시글 최근작성 순 상위 3개 포스트 뿌리기
	PostListDao pDao = new PostListDao();
	ArrayList<MainpostListDto> mainPostList = pDao.getMainPostList(); 
	
	CategoryDao cDao = new CategoryDao();
	ArrayList<CategoryDto> categoryList = cDao.getCategorySelect();
	 
	MainDao mDao = new MainDao();
	ArrayList<MainRandomGosuDto> mList = mDao.getRandomGosu();
	
// 	int userIdx = Integer.parseInt(request.getParameter("userIDx"));
	NoticeDao nDao = new NoticeDao();
	ArrayList<NoticeDto> nList = nDao.getNoticeList(userIdx);  // 알림 리스트를 가져올 메서드
	
	ServiceDao sDao = new ServiceDao();
	String searchString = request.getParameter("searchText");
	ArrayList<ServiceList> sList = sDao.getServiceList(searchString);
...
<meta charset="UTF-8">
	...
	$(document).on("click", ".usermenu-dropdown-div3-button", function(){
		location.reload();
	});
		...
		<div id="app-body" class="center">
			<div id="home">
				...
				<div id="main-pro-advertisement-container" class="center">
					...
					<%for(MainRandomGosuDto list : mList) { %>
				...
				<div id="main-community-list-container" class="center">
					...
					<div id="main-community-contents" class="center">
						<div id="curation-container-left">
							<ul>
								<li>
								<%for(MainpostListDto dto : mainPostList) { %>
									<a class="curation-list" idx="<%=dto.getPostIdx()%>">
										<div class="curation-contents-thumbnail">
											...
											<%ArrayList<MainPostActionDto> actionList = pDao.getMainAction(dto.getPostIdx());%>
											<%for(MainPostActionDto actionDto : actionList) { %>
						...
						<%for (GosuKnowhowPostListDto dto : mainList) {%>
